...
\def\TaskWidth{3.3in}            % width of "Task" column (leftmost column)
\def\LeadWidth{0.45in}           % width of "Task Lead" column
\def\ExpertiseWidth{1.2in}       % width of "Team Expertise" column (rightmost column)

%---- table compactness controls
\def\SpaceBetweenRows{0.7}       % vertical compactness of rows
...
\def\TaskCategoryColor{Blue}              % task grouping banner color
\def\TaskCategoryFontColor{White}         % task grouping title font color
...
\def\TaskCategoryFontstyle#1{\textbf{#1}} % boldface task grouping title
...
\newcolumntype{T}{
  ...
  c!{\color{\YearQuarterLineColor}\vrule} % Start/year
  c!{\color{\YearQuarterLineColor}\vrule}!{\color{\YearQuarterLineColor}\vrule} % Start/quarter
  c!{\color{\YearQuarterLineColor}\vrule} % Finish/year
  c||                                     % Finish/quarter
...
[STARTBANNER]
\multicolumn{7}{|c|}{\cellcolor{\TaskCategoryColor}\color{\TaskCategoryFontColor}\TaskCategoryFontstyle{Task [INSERTCATEGORYID]: [INSERTCATEGORYTITLE]}}\\
[ENDBANNER]
